...
let _ =
  ...
  let cipher = Test_Machine.encrypt test_steckerbrett initial_position (Enigml.decompose plaintext) in
  print_string "Ciphertext: ";
  Enigml.print_letters cipher;
  print_newline ();
  print_string "Decryption: ";
  Enigml.print_letters @@ Test_Machine.encrypt test_steckerbrett initial_position cipher;
  print_newline ()
